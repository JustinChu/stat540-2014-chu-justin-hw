...
library(plyr)
library(car)
prDat <- read.table("GSE7191-data.txt")
...
Fix date and genotype for prDes:
```{r}
prDes$DateRun <- as.Date(prDes$DateRun, format="%m/%d/%y" )
prDes$Genotype <- recode(prDes$Genotype, "'Wild_type'='wt'; 'S1P2_KO'='S1P2KO'; 'S1P3_KO'='S1P3KO'", as.factor.result = FALSE)
...
```
Seems fairly balanced for each tissue type. If comparing genotypes wt, S1P2KO and S1P3KO, the latter is lacking in samples for each tissue type compared to the others. It is nice to see that wt and S1P2KO are at least balanced.

```{r}
table(prDes$DateRun, prDes$Genotype)
```
Most of the wt controls were obtained earlier in addition to in consistancies of when each genotype as collected each day. These differences could confount the results. Optimally each date at would have the same number of collected samples.


```{r}
table(prDes$DateRun, prDes$BrainRegion)
```
More balanced than genetype collection consitance. There seems to be more sample collect near the latter dates that could affect the results.
