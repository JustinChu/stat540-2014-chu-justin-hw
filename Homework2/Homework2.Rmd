...
```{r}
bothDat <- intersect(edgerTopGenes$gene.id, arrayDE$gene.id)
arrayDatBoth <- arrayDE[arrayDE$gene.id %in% bothDat,]
...
p <- ggplot(arrayDatBoth, aes(x=q.value, colour = type, fill = type)) + geom_density(alpha =0.5) + scale_x_log10()
p
```
...
```{r}
allDat <- union(edger.results$gene.id, arrayDE$gene.id)
